...
int AggressiveStrategy::strategyDeclineDecision(Players* inputPlayer)
{
	if (inputPlayer->get_in_hand_solider_current_race() <= 1) return 1;
	else return 0;
}

int AggressiveStrategy::strategyRaceSelection(Players* inputPlayer)
{
	...
	int AggressiveStrategy::strategyRegionSelection(Players* inputPlayer)
	{
		if (!(inputPlayer))
		{
